...
Descripción de los datos: La Agencia Nacional de Investigación y Desarrollo (ANID) cada año adjudica financiamiento para proyectos en Ciencia y Tecnología a través de sus diferentes concursos.
La base de datos denominada "BDH_Proyectos" contiene la información disponible de proyectos adjudicados por la Agencia (antes del 2020, CONICYT) desde el año 1982 hasta el 2020, con fecha de corte al 31 de diciembre del 2020.
Cada fila representa una iniciativa adjudicada.
...
2)  Carga la base de datos con el IPC anual y guárdala en un objecto llamado `datos_ipc`. Remueve los años con valores perdidos en la variable `datos_ipc$ipc`, usa. Conserva sólo las variables `anno` e `ipc`. Los datos deben verse así:
...
tabla_1 %>% 
  pivot_wider(names_from=sexo, values_from=monto_precios2021) %>%
  mutate(across(c(HOMBRE,MUJER), ~ ifelse(is.na(.x),0,.x) ))
