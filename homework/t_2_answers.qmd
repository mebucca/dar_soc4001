...
Debes obtener un resultado similar al que se muestra después de cada pregunta.
...
```{r,echo=FALSE}
resultados <-  datos_chile %>% group_by(region,education,cohort73) %>%
                summarise(across(c("vote_no", "income"), list(media = ~mean(.x, na.rm = TRUE), sd= ~sd(.x, na.rm = TRUE)) ))
